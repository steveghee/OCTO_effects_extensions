function twxSeeFlow() {
  return {
    ...
    runtimeTemplate: function (props, twxWidgetEl, fullOriginalDoc, $, projectSettings) {
      ...
      var ps2 = '<script name="reflectoflow_gl" type="x-shader/x-fragment"> precision mediump float; varying vec3 I; varying vec3 N; varying vec2 T; const float PI = 3.1415; uniform mat4 inverseViewMatrix; uniform sampler2D texSampler2D; uniform sampler2D envSampler2D; uniform vec4 surfaceColor; uniform float intensity; uniform float tick; uniform float mixer; uniform float envrotate; uniform float du; uniform float dv; uniform float sc1; uniform float sc2; const vec4 ambientColor = vec4(0.15, 0.15, 0.15, 1.0); const vec4 specColor = vec4(1.5, 1.5, 1.5, 1.); void main() { vec3 NN = normalize(N); vec3 II = normalize(I); vec3 R = normalize(reflect(II, NN)); vec3 RR = vec3(inverseViewMatrix*vec4(R,0.)); float tx = envrotate-atan(RR.x, RR.z) / (2.0 * PI); float ty = .5+(asin(-RR.y) / PI); vec2 tex = vec2(tx,ty); vec3 LP = vec3(1.,1.,1.); vec3 LL = normalize(LP); float lambertian = dot(LL,NN); float specular = 0.0; if (lambertian < 0.0) NN = - NN; vec3 reflectDir = reflect(-LL, NN); float specAngle = max(dot(reflectDir, II), 0.0); specular = pow(specAngle, 10.0); vec4 color = specColor * specular;color.a = 1.; float dudt = mod(-du*tick, 1./sc1); float dvdt = mod(-dv*tick, 1./sc2); vec2 st = vec2((sc1*(T.x+dudt)),(sc2*(T.y+dvdt))); vec4 fcolor = color+mix(texture2D(texSampler2D,st),texture2D(envSampler2D, tex),intensity); fcolor.a = fcolor.a*intensity; gl_FragColor = fcolor; } </script>';
