function twxFlipbookAnimation() {
 ...
 isBindingTarget: false,
    ...
    runtimeTemplate: function (props, twxWidgetEl, fullOriginalDoc, $, projectSettings) {
      ...
      var ps1='<script name="flipper_gl" type="x-shader/x-fragment"> precision mediump float; varying vec2 texCoord; uniform sampler2D texSampler2D; uniform float tick; uniform float intensity; uniform float blend; uniform float rate; uniform float direction; uniform float frames; uniform vec4 surfaceColor; void main(void) { float uwidth = 1. / frames; float vis = intensity; float speed = (direction >= 0.) ? 1.+rate : -1.-rate; float mt = mod(speed * tick,1./uwidth); float lt = uwidth * (floor(mt) + texCoord.x); vec4 tx1 = texture2D(texSampler2D,vec2(lt,texCoord.y)); vec4 flip = vec4(tx1.rgb,tx1.a*vis); vec4 color = mix(surfaceColor,tx1,tx1.a*vis); gl_FragColor = mix(flip,color,blend); if (gl_FragColor.a <= 0.) discard; } </script>';
